...
function renderStepActions(step) {
  ...
  if(sid==='income') {
    ...
    // _reinfIncome is set by applyIncome which runs in executeAutoStep after this render
    // Use pendingSoldiers which is always current
...
function applyExplosions(box, modal, fk, nucTers, rolls, R, explosionCount, isMyMaint, callback) {
  ...
  // Player: select nuclears TO DESTROY directly on the MAP
  const allMyNucs = Object.values(G.territories).filter(t=>t.owner===G.pf&&t.hasNuclear);
  const chosen = new Set();

  // Close the maintenance modal - player interacts with map
  modal.remove();

  // Show overlay instruction banner
  ...
  instrBanner.innerHTML = '<div style="font-size:11px;color:#ff4444;letter-spacing:2px;margin-bottom:4px;">ELIGE '+explosionCount+' NUCLEAR A DESTRUIR</div>'+
    ...
    '<button id="nuke-destroy-confirm" disabled onclick="confirmNukeDestroy()" style="margin-top:8px;background:#0a0a0d;border:1px solid #ff4444;color:#ff4444;padding:6px 18px;font-family:Orbitron,sans-serif;font-size:9px;letter-spacing:2px;cursor:pointer;opacity:0.4;">CONFIRMAR</button>';
  document.body.appendChild(instrBanner);

  // Highlight selectable nuclear territories orange on map
  allMyNucs.forEach(t=>{
  ...
  // Store state for map click handler
...
function onTerritoryClick(id, event) {
  ...
  // Nuclear destroy mode (explosion selection on map)
